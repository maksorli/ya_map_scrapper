    ...
    with webdriver.Chrome(options=options_chrome) as browser:
        ...
        WebDriverWait(browser, 10).until(
            EC.element_to_be_clickable(
                (By.CSS_SELECTOR, 'input[class="CheckboxCaptcha-Button"]')
            )
        ).click()
        ...
        with open(img_name, "wb") as file:
            ...
            img = browser.find_element(
                By.CSS_SELECTOR, 'img[class="AdvancedCaptcha-Image"]'
            ).get_attribute("src")
        ...
        browser.find_element(
            By.CSS_SELECTOR, 'input[class="Textinput-Control"]'
        ).send_keys(dict_result["code"])
        ...
        browser.find_element(
            By.CSS_SELECTOR, 'button[class="CaptchaButton CaptchaButton_view_action"]'
        ).click()
